...
if (isset($_POST['date']) && isset($_POST['order'])) {
    ...
    if ($date) {
        $sql = "SELECT Nosaukums, Datums, Laiks, Informacija, Cena, Plakats FROM festivali WHERE Datums = '$date' ORDER BY Datums $order";
    } else {
        $sql = "SELECT Nosaukums, Datums, Laiks, Informacija, Cena, Plakats FROM festivali ORDER BY Datums $order";
    }
