...
if (isset($_POST['date']) && isset($_POST['order'])) {
    ...
    if ($date) {
        $sql = "SELECT CitiID, Nosaukums, Datums, Laiks, Informacija, Cena, Plakats FROM citi WHERE Datums = '$date' ORDER BY Datums $order";
    } else {
        $sql = "SELECT CitiID, Nosaukums, Datums, Laiks, Informacija, Cena, Plakats FROM citi ORDER BY Datums $order";
    }
    ...
    if (count($events) > 0) {
        foreach ($events as $event) {
            ...
            echo '    <a href="citi_details.php?id=' . htmlspecialchars($event["CitiID"]) . '">';
